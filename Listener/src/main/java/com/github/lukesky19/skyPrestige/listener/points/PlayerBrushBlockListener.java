...
public class PlayerBrushBlockListener extends PrestigePointsListener<BlockDropItemEvent> {
    ...
    @Override
    protected void handle(@NotNull Settings settings, @NotNull IslandData islandData, @NotNull BlockDropItemEvent blockDropItemEvent, @NotNull EventContext eventContext) {
        @Nullable BlockType blockType = eventContext.getBlockType();
        if(blockType == null) return;

        @Nullable Double prestigePoints = settings.prestigePointsMapping().getBrushPrestigePoints(blockType);
