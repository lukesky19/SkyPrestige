...
public class OptOutRewardsGUI extends ChestGUI<IslandIdUUIDKey> {
    ...
    public OptOutRewardsGUI(
            ...
            @NotNull Player player,
            @NotNull GUIConfigManager guiConfigManager,
            @NotNull OptInOutConfig optOutConfig,
            @Nullable ConfirmOptOutGUI confirmOptOutGUI) {
        super(plugin, guiManager, identifier, player);
    ...
    public boolean create() {
        if(optOutConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the opt-out rewards GUI due to invalid opt-out configuration."));
            return false;
        }

        if(rewardsGUIConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the opt-out rewards GUI due to invalid opt-out rewards GUI configuration."));
            return false;
        }

        GUIType guiType = rewardsGUIConfig.guiType();
        if(guiType == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the opt-out rewards GUI due to an invalid GUIType."));
            return false;
        }
