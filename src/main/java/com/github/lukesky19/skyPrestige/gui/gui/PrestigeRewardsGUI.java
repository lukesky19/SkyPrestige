 ...
 */
public class PrestigeRewardsGUI extends ChestGUI<IslandIdUUIDKey> {
    // The ConfirmPrestigeGUI to open after this one is closed.
    // This GUI can also be opened via a command so they don't always have a ConfirmPrestigeGUI to go back to.
    ...
    public PrestigeRewardsGUI(
        ...
        this.rewardsGUIConfig = guiConfigManager.getPrestigeRewardsGUIConfig();
    ...
    public boolean update() {
        ...
        createRewardsButtons(prestigeConfig);

        rewardsAddedPerPage.put(pageNum, numOfRewardsAdded);
        errorCountsPerPage.put(pageNum, numOfRewardsErrored);

        int totalRewardsCount = getTotalRewardsCount(prestigeConfig.rewardConfig());
    ...
    private void createExitButton() {
        ...
        if(exitConfig.slot() == null) {
            logger.warn(AdventureUtility.plain("Unable to add the exit button to the rewards GUI due to an invalid slot."));
            return;
        }
     ...
     */
    private void createRewardsButtons(@NonNull PrestigeConfig prestigeConfig) {
    ...
    private void addRewardButton(@NonNull IReward reward) {
        ...
        if(itemStackConfig.itemType() != null) {
            ...
            if(optionalItemStack.isPresent()) {
                ...
                logger.warn(AdventureUtility.plain("Unable to add a reward display item to the Rewards GUI due to a null ItemStack."));
                numOfRewardsErrored++;
            }
        }
    }
    ...
    private void createDummyButtons() {
        ...
        rewardsGUIConfig.dummyButtons().forEach(buttonConfig -> {
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to add a dummy button to the rewards GUI due to an invalid slot."));
                return;
            }
