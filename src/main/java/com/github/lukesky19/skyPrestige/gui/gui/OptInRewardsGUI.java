...
/**
 * This class is used to create the GUI to view rewards for opting in.
 */
public class OptInRewardsGUI extends ChestGUI<IslandIdUUIDKey> {
    // The ConfirmOptInGUI to open after this one is closed.
    // This GUI can also be opened via a command so they don't always have a ConfirmOptInGUI to go back to.
    private final @Nullable ConfirmOptInGUI confirmOptInGUI;
     ...
     * @param player The {@link Player} viewing the GUI.
     * @param guiConfigManager A {@link GUIConfigManager} instance.
     * @param optInConfig The {@link OptInOutConfig} for opt-in.
     * @param confirmOptInGUI The {@link ConfirmOptInGUI} the player came from, if any.
     */
    public OptInRewardsGUI(
            @NotNull SkyPlugin plugin,
    ...
    public boolean create() {
        if(optInConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the opt-in rewards GUI due to invalid opt-in configuration."));
            return false;
        }

        if(rewardsGUIConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the opt-in rewards GUI due to invalid opt-in rewards GUI configuration."));
            return false;
        }

        GUIType guiType = rewardsGUIConfig.guiType();
        if(guiType == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the opt-in rewards GUI due to an invalid GUIType."));
            return false;
        }
        ...
        switch(guiType) {
            ...
            default -> {
                logger.error(AdventureUtil.deserialize("Unsupported GUI Type in opt-in rewards GUI config. Allowed Types: CHEST_27, CHEST_36, CHEST_45, CHEST_54"));
                return false;
            }
        }
    ...
    public boolean update() {
        ...
        if(optInConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to add buttons to the GUI due to invalid opt-in configuration."));
            return false;
        }
        ...
        createRewardsButtons(optInConfig.rewardConfig());

        rewardsAddedPerPage.put(pageNum, numOfRewardsAdded);
        errorCountsPerPage.put(pageNum, numOfRewardsErrored);

        int totalRewardsCount = getTotalRewardsCount(optInConfig.rewardConfig());
    ...
    public void handleClose(@NotNull InventoryCloseEvent inventoryCloseEvent) {
        ...
        if(confirmOptInGUI != null) confirmOptInGUI.open();
    }
    ...
    private void createNextPageButton() {
        ...
        if(nextPageConfig.slot() == null) {
            logger.warn(AdventureUtil.deserialize("Unable to add the next page button to the opt-in rewards GUI due to an invalid slot."));
            return;
        }
    ...
    private void createPrevPageButton() {
        ...
        if(prevPageConfig.slot() == null) {
            logger.warn(AdventureUtil.deserialize("Unable to add the previous page button to the opt-in rewards GUI due to an invalid slot."));
            return;
        }
    ...
    private void createExitButton() {
        ...
        if(exitConfig.slot() == null) {
            logger.warn(AdventureUtil.deserialize("Unable to add the exit button to the opt-in rewards GUI due to an invalid slot."));
            return;
        }
        ...
        createActionButton(exitConfig, inventoryClickEvent ->
                plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                    ...
                    if(confirmOptInGUI != null) {
                        confirmOptInGUI.open();
                    }
                }, 1L));
    }
    ...
    private void createDummyButtons() {
        ...
        rewardsGUIConfig.dummyButtons().forEach(buttonConfig -> {
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to add a dummy button to the opt-in rewards GUI due to an invalid slot."));
                return;
            }
