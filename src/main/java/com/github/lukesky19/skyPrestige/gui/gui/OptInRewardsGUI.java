...
public class OptInRewardsGUI extends ChestGUI<IslandIdUUIDKey> {
    ...
    public boolean create() {
        if(optInConfig == null) {
            logger.warn(AdventureUtility.plain("Unable to create the InventoryView for the opt-in rewards GUI due to invalid opt-in configuration."));
            return false;
        }

        if(rewardsGUIConfig == null) {
            logger.warn(AdventureUtility.plain("Unable to create the InventoryView for the opt-in rewards GUI due to invalid opt-in rewards GUI configuration."));
            return false;
        }

        GUIType guiType = rewardsGUIConfig.guiType();
        if(guiType == null) {
            logger.warn(AdventureUtility.plain("Unable to create the InventoryView for the opt-in rewards GUI due to an invalid GUIType."));
            return false;
        }
