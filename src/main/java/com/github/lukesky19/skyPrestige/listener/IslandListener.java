...
public class IslandListener implements Listener {
    ...
    public void onIslandReset(IslandResetEvent islandResetEvent) {
        ...
        String oldIslandId = islandResetEvent.getOldIsland().getUniqueId();
        String newIslandId = islandResetEvent.getIsland().getUniqueId();

        // Remove island id from list of prestiged island ids if the event is cancelled
        if(islandResetEvent.isCancelled()) {
        ...
        if(settings != null && settings.resetPrestigeLevelOnIslandReset()) islandData.setPrestigeLevel(0);
