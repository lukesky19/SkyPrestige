...
public class PlayerDropItemListener extends PrestigePointsListener<EntityDropItemEvent> {
    ...
    @Override
    protected void handle(@NotNull PrestigePointsConfig prestigePointsConfig, @NotNull IslandData islandData, @NotNull EntityDropItemEvent entityDropItemEvent, @NotNull EventContext eventContext) {
        ...
        if(enchantments != null && !enchantments.isEmpty()) {
            ...
            for(Map.Entry<Enchantment, Integer> enchantmentEntry : enchantments.entrySet()) {
                @Nullable Double points = prestigePointsConfig.prestigePointsMapping().getItemDropPrestigePoints(itemType, enchantmentEntry.getKey(), enchantmentEntry.getValue());
                if(points != null) totalPoints += points;
            }

            if(totalPoints > 0) prestigePoints = totalPoints;
        } else if(potionType != null) {
            prestigePoints = prestigePointsConfig.prestigePointsMapping().getItemDropPrestigePoints(itemType, potionType);
        } else {
            prestigePoints = prestigePointsConfig.prestigePointsMapping().getItemDropPrestigePoints(itemType);
        }
