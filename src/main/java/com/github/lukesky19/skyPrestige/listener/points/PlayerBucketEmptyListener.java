 ...
 */
public class PlayerBucketEmptyListener extends PointsListener {
    /**
     * Constructor
    ...
    public void onBucketFill(PlayerBucketEmptyEvent playerBucketEmptyEvent) {
        ...
        // Item
        ItemType itemType = playerBucketEmptyEvent.getBucket().asItemType();
        if(itemType == null) return;

        // Points
        double points = prestigePointsManager.getItemPoints(ActionType.EMPTY, prestigePointsConfig.prestigePointsMapping().empty(), itemType, null, null, null);
        if(points <= 0) return;
