...
public class PlayerBoneMealListener extends PrestigePointsListener<BlockFertilizeEvent> {
    ...
    @Override
    protected void handle(@NotNull PrestigePointsConfig prestigePointsConfig, @NotNull IslandData islandData, @NotNull BlockFertilizeEvent blockFertilizeEvent, @NotNull EventContext eventContext) {
        @Nullable BlockType blockType = eventContext.getBlockType();
        if(blockType == null) return;

        @Nullable Double prestigePoints = prestigePointsConfig.prestigePointsMapping().getBoneMealPrestigePoints(blockType);
