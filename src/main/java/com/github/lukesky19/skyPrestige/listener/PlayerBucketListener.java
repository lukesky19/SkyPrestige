...
/**
 * Listens for when a player fills or empties a bucket on an island and increments prestige points.
 */
public class PlayerBucketListener implements Listener {
    ...
    /**
     * Listens for when a player fills a bucket on an island and increments prestige points.
    ...
    public void onBucketScoop(PlayerBucketFillEvent playerBucketFillEvent) {
        ...
        ItemStack resultStack = playerBucketFillEvent.getItemStack();
        if(resultStack == null) return;
        ItemType resultBucket = resultStack.getType().asItemType();
        if(resultBucket == null) return;
        ...
        @Nullable Double prestigePoints = settings.prestigePointsMapping().getFillPrestigePoints(resultBucket);
     ...
     * @param playerBucketEmptyEvent A {@link PlayerBucketFillEvent}.
     */
    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onBucketEmpty(PlayerBucketEmptyEvent playerBucketEmptyEvent) {
        ...
        if(player.getGameMode().equals(GameMode.CREATIVE)) return;
        UUID uuid = player.getUniqueId();
        ItemType sourceBucket = playerBucketEmptyEvent.getBucket().asItemType();
        if(sourceBucket == null) return;
        ItemStack resultStack = playerBucketEmptyEvent.getItemStack();
        if(resultStack == null) return;
        ItemType resultBucket = resultStack.getType().asItemType();
        if(resultBucket == null) return;
        ...
        @Nullable Double prestigePoints = settings.prestigePointsMapping().getEmptyPrestigePoints(resultBucket);
        if(prestigePoints == null) return;
