...
/**
 * Listens for when a player closes an Inventory attached to a block on an Island and increments prestige points.
 */
public class InventoryCloseListener extends PrestigePointsListener<InventoryCloseEvent> {
    /**
     * Constructor
    ...
    public void onInventoryClosed(InventoryCloseEvent inventoryCloseEvent) {
        process(inventoryCloseEvent);
    }

    @Override
    protected @NotNull EventContextExtractor<InventoryCloseEvent> extractor() {
    ...
    @Override
    protected void handle(@NotNull PrestigePointsConfig prestigePointsConfig, @NotNull IslandData islandData, @NotNull InventoryCloseEvent inventoryCloseEvent, @NotNull EventContext eventContext) {
        @Nullable BlockType blockType = eventContext.getBlockType();
        if(blockType == null) return;

        @Nullable Double prestigePoints = prestigePointsConfig.prestigePointsMapping().getClosePrestigePoints(blockType);
