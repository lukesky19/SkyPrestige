...
public class PlayerThrowItemListener implements Listener {
    ...
    /**
     * Listens for when a player throws an item (which becomes an entity) on an island and increments prestige points.
     * @param projectileLaunchEvent A {@link PlayerLaunchProjectileEvent}.
     */
    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onPlayerThrownEntity(PlayerLaunchProjectileEvent projectileLaunchEvent) {
        Settings settings = settingsManager.getSettings();
        if(settings == null) return;

        Player player = projectileLaunchEvent.getPlayer();
        if(player.getGameMode().equals(GameMode.CREATIVE)) return;
        UUID uuid = player.getUniqueId();
        ItemStack itemStack = projectileLaunchEvent.getItemStack();
        ...
        if(itemMeta instanceof PotionMeta potionMeta) {
            ...
            if(potionType != null) {
                @Nullable Double prestigePoints = settings.prestigePointsMapping().getThrownPrestigePoints(itemType, potionType);
        ...
        @Nullable Double prestigePoints = settings.prestigePointsMapping().getThrownPrestigePoints(itemType);
