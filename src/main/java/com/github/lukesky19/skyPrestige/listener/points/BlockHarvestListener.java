...
/**
 * Listens for when a player harvests a block on an island and increments prestige points.
 */
public class BlockHarvestListener extends PrestigePointsListener<PlayerHarvestBlockEvent> {
    /**
     * Constructor
    ...
    public void onBlockHarvest(PlayerHarvestBlockEvent playerHarvestBlockEvent) {
        process(playerHarvestBlockEvent);
    }

    @Override
    protected @NotNull EventContextExtractor<PlayerHarvestBlockEvent> extractor() {
        return playerHarvestBlockEvent -> {
            Block block = playerHarvestBlockEvent.getHarvestedBlock();
            BlockType blockType = block.getType().asBlockType();
            if(blockType == null) return null;

            EventContext eventContext = new EventContext();
            eventContext.setPlayer(playerHarvestBlockEvent.getPlayer());
    ...
    @Override
    protected void handle(@NotNull PrestigePointsConfig prestigePointsConfig, @NotNull IslandData islandData, @NotNull PlayerHarvestBlockEvent playerHarvestBlockEvent, @NotNull EventContext eventContext) {
        @Nullable BlockType blockType = eventContext.getBlockType();
        if(blockType == null) return;

        @Nullable Double prestigePoints = prestigePointsConfig.prestigePointsMapping().getHarvestPrestigePoints(blockType);
