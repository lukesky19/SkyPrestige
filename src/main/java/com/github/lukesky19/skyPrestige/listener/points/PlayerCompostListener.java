 ...
 */
public class PlayerCompostListener extends PointsListener {
    /**
     * Constructor
    ...
    public void onPlayerCompost(EntityCompostItemEvent entityCompostItemEvent) {
        ...
        // Player
        if(!(entityCompostItemEvent.getEntity() instanceof Player player)) return;
        ...
        // Item
        @NotNull ItemStack itemStack = entityCompostItemEvent.getItem();
        @Nullable ItemType itemType = itemStack.getType().asItemType();
        if(itemType == null) return;

        // Points
        double points = prestigePointsManager.getItemPoints(ActionType.COMPOST, prestigePointsConfig.prestigePointsMapping().compost(), itemType, null, null, null);
        if(points <= 0) return;
