...
public class PlayerCompostListener extends PrestigePointsListener<EntityCompostItemEvent> {
    ...
    @Override
    protected void handle(@NotNull PrestigePointsConfig prestigePointsConfig, @NotNull IslandData islandData, @NotNull EntityCompostItemEvent entityCompostItemEvent, @NotNull EventContext eventContext) {
        @Nullable ItemType itemType = eventContext.getItemType();
        if(itemType == null) return;

        @Nullable Double prestigePoints = prestigePointsConfig.prestigePointsMapping().getCompostPrestigePoints(itemType);
