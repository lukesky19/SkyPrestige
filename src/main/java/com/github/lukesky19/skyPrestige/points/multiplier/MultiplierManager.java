...
*/
package com.github.lukesky19.skyPrestige.points.multiplier;
...
public class MultiplierManager {
    ...
    public void startEvent(boolean broadcast) {
        ...
        if(eventDuration > 0) {
            skyPrestige.getComponentLogger().warn(AdventureUtil.deserialize("Unable to start event because one is already running."));
            return;
        }
        ...
        if(broadcast) {
            ...
            skyPrestige.getServer().getOnlinePlayers().forEach(player -> player.sendMessage(message));
        }
    }
