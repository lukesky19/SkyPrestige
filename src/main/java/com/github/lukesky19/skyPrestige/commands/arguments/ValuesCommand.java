    ...
    public @NonNull LiteralCommandNode<CommandSourceStack> createCommand() {
        return Commands.literal("values")
                ...
                .executes(ctx -> {
                    ...
                    if(!creationResult) {
                        logger.error(AdventureUtility.plain("Unable to create the InventoryView for the values GUI for player " + player.getName() + " due to a configuration error."));
                    ...
                    if(!updateResult) {
                        logger.error(AdventureUtility.plain("Unable to decorate the values GUI for player " + player.getName() + " due to a configuration error."));
                        player.sendMessage(AdventureUtility.deserialize(locale.prefix() + locale.guiOpenError()));
                        return 0;
                    }
                    ...
                    if(!openResult) {
                        logger.error(AdventureUtility.plain("Unable to open the values GUI for player " + player.getName() + " due to a configuration error."));
                        player.sendMessage(AdventureUtility.deserialize(locale.prefix() + locale.guiOpenError()));
                        return 0;
                    }
