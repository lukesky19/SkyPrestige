    ...
    public @NotNull LiteralCommandNode<CommandSourceStack> createCommand() {
        return Commands.literal("progress")
                ...
                .executes(ctx -> {
                    ...
                    if(!creationResult) {
                        logger.error(AdventureUtil.deserialize("Unable to create the InventoryView for the progress GUI for player " + player.getName() + " due to a configuration error."));
                    ...
                    if(!updateResult) {
                        logger.error(AdventureUtil.deserialize("Unable to decorate the progress GUI for player " + player.getName() + " due to a configuration error."));
                        player.sendMessage(AdventureUtil.deserialize(locale.prefix() + locale.guiOpenError()));
                        return 0;
                    }
                    ...
                    if(!openResult) {
                        logger.error(AdventureUtil.deserialize("Unable to open the progress GUI for player " + player.getName() + " due to a configuration error."));
                        player.sendMessage(AdventureUtil.deserialize(locale.prefix() + locale.guiOpenError()));
                        return 0;
                    }
