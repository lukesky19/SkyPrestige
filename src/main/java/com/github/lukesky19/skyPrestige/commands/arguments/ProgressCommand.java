...
import com.github.lukesky19.skyPrestige.gui.ProgressGUI;
...
/**
 * This class creates the progress command argument for the skyprestige command.
 */
public class ProgressCommand {
    ...
    /**
     * Creates the {@link LiteralCommandNode} of type {@link CommandSourceStack} for the progress command argument for the /skyprestige command.
     * @return A {@link LiteralCommandNode} of type {@link CommandSourceStack} for the progress command argument for the /skyprestige command.
     */
        ...
        return Commands.literal("progress")
                ...
                .executes(ctx -> {
                    ...
                    if(nextPrestigeLevelConfig == null) {
                        player.sendMessage(AdventureUtil.serialize(locale.prefix() + locale.progressMaxPrestigeLevel()));
                        return 0;
                    }

                    // Create the ProgressGUI
                    ProgressGUI gui = new ProgressGUI(skyPrestige, settingsManager, localeManager, guiConfigManager, guiManager, player, island, islandData, nextPrestigeLevelConfig);

                    boolean creationResult = gui.create();
                    if(!creationResult) {
                        logger.error(AdventureUtil.serialize("Unable to create the InventoryView for the progress GUI for player " + player.getName() + " due to a configuration error."));
                    ...
                    if(!updateResult) {
                        logger.error(AdventureUtil.serialize("Unable to decorate the progress GUI for player " + player.getName() + " due to a configuration error."));
                        player.sendMessage(AdventureUtil.serialize(locale.prefix() + locale.guiOpenError()));
                        return 0;
                    }
                    ...
                    if(!openResult) {
                        logger.error(AdventureUtil.serialize("Unable to open the progress GUI for player " + player.getName() + " due to a configuration error."));
                        player.sendMessage(AdventureUtil.serialize(locale.prefix() + locale.guiOpenError()));
                        return 0;
                    }
