...
*/
package com.github.lukesky19.skyPrestige.config.manager.gui;

import com.github.lukesky19.skyPrestige.SkyPrestige;
import com.github.lukesky19.skyPrestige.config.data.gui.*;
...
public class GUIConfigManager {
    ...
    public void reload() {
        ...
        try {
            progressGUIConfig = progressLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.ProgressGUIConfig.class);
        } catch (ConfigurateException configurateException) {
            logger.error(AdventureUtil.deserialize("Failed to load progress GUI config. Error:" + configurateException.getMessage()));
        }

        try {
            blueprintGUIConfig = blueprintsLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.BlueprintGUIConfig.class);
        } catch (ConfigurateException configurateException) {
            logger.error(AdventureUtil.deserialize("Failed to load blueprints GUI config. Error:" + configurateException.getMessage()));
        }

        try {
            confirmPrestigeGUIConfig = confirmPrestigeLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.ConfirmPrestigeGUIConfig.class);
        ...
        try {
            rewardsGUIConfig = rewardsLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.RewardsGUIConfig.class);
        } catch (ConfigurateException configurateException) {
            logger.error(AdventureUtil.deserialize("Failed to load rewards GUI config. Error:" + configurateException.getMessage()));
        }

        try {
            exchangeGUIConfig = exchangeLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.ExchangeGUIConfig.class);
        } catch (ConfigurateException configurateException) {
            logger.error(AdventureUtil.deserialize("Failed to load the exchange GUI config. Error:" + configurateException.getMessage()));
        }

        try {
            vaultGUIConfig = vaultLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.VaultGUIConfig.class);
        } catch (ConfigurateException configurateException) {
            logger.error(AdventureUtil.deserialize("Failed to load the vault GUI config. Error:" + configurateException.getMessage()));
        }

        try {
            valuesGUIConfig = valuesLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.ValuesGUIConfig.class);
        } catch (ConfigurateException configurateException) {
            logger.error(AdventureUtil.deserialize("Failed to load the values GUI config. Error:" + configurateException.getMessage()));
        }

        try {
            infoGUIConfig = infoLoader.load().get(com.github.lukesky19.skyPrestige.config.data.gui.InfoGUIConfig.class);
        } catch (ConfigurateException configurateException) {
    ...
    private void updateConfirmGUIConfig() {
        ...
        switch(confirmPrestigeGUIConfig.configVersion()) {
            ...
            case "1.0.0.0" -> {
                ...
                this.confirmPrestigeGUIConfig = new com.github.lukesky19.skyPrestige.config.data.gui.ConfirmPrestigeGUIConfig(
                        "1.1.0.0",
                ...
                try {
                    ...
                    node.set(com.github.lukesky19.skyPrestige.config.data.gui.ConfirmPrestigeGUIConfig.class, confirmPrestigeGUIConfig);
