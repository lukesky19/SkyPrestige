...
public class RewardsGUI extends ChestGUI {
        ...
        switch(reward) {
            ...
            default -> {
                    logger.warn(AdventureUtil.deserialize("Unable to add a reward display item as the " +
                            "reward is not an ItemReward, CommandReward, MoneyReward, or IslandRangeReward."));
                    return;
            }
        }
    ...
    private @Nullable PrestigeConfig.Reward getCurrentReward(@NotNull PrestigeConfig.RewardConfig rewardConfig) {
        ...
        if(currentRewardKey < itemRewardsCount) {
            ...
            return rewardConfig.moneyRewards().get(currentRewardKey - itemRewardsCount - commandRewardsCount);
        } else if(rewardConfig.islandRangeReward().rangeToAdd() > 0
                && currentRewardKey == itemRewardsCount + commandRewardsCount + moneyRewardsCount) {
            return rewardConfig.islandRangeReward();
        }
